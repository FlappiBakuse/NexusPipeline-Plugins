...
- data-specialized 插件的 resolve、judgeScript 以及可选旧 configValidator/configEditor 均位于 `data/`，能力仅来自三个 Host 白名单且无 `frontend` 字段；taskProtocol 1.2 另要求 `data/i18n/` 词典、声明式 `configRules`/`environmentChecks`，并禁止 configValidator；公开 configEditor 字段不变但官方资产使用 `data/editor.js`；
