...
class CandidateSourceTests(unittest.TestCase):
    def fixture(self):
        run = {"id": 12, "repository": {"full_name": REPO}, "workflow_id": 88,
               "path": ".github/workflows/publish-develop.yml", "event": "workflow_dispatch", "head_branch": "main",
               "head_sha": SHA, "conclusion": "failure"}
        current = {"id": 99, "repository": {"full_name": REPO}, "workflow_id": 88}
        artifact = {"id": 42, "name": "plugins-develop-preview-12-2", "expired": False,
        ...
        paths = {
            ...
            f"{PREFIX}/runs/12/attempts/2/jobs?per_page=100":
                {"jobs": [{"name": "preview-build", "status": "completed", "conclusion": "success"}]},
        }
        return paths
    ...
    def test_original_job_success_survives_writer_workflow_failure(self):
        ...
        self.assertEqual((result["workflowSha"], result["runAttempt"], result["artifactId"]), (SHA, 2, 42))
    ...
    def test_ambiguous_artifacts_require_exact_id(self):
        ...
        other.update(id=43, name="plugins-develop-preview-12-3")
