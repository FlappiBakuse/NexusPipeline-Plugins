...
JOB_NAME = "Build and validate Plugins candidate"
    ...
    *,
    ...
    for item in artifacts:
        if not isinstance(item, dict):
            continue
        match = re.fullmatch(rf"plugins-stable-candidate-{candidate_run_id}-([1-9][0-9]*)", str(item.get("name", "")))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Resolve original Plugins candidate producer and artifact")
