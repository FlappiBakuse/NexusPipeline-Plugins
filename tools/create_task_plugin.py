...
def generate(artifact, name, example=False, preset="json-id-array", protocol_version="1.1"):
    ...
    if protocol_version not in ('1.0', '1.1', '1.2'): raise ValueError('Unsupported protocol version')
...
def main():
    ...
    p.add_argument('--preset',choices=PRESETS,default='json-id-array');p.add_argument('--protocol-version',choices=('1.0','1.1','1.2'),default='1.1');p.add_argument('--output',type=Path,required=True);p.add_argument('--example',action='store_true');p.add_argument('--check',action='store_true');a=p.parse_args()
