...
def generate(artifact, name, example=False, preset="json-id-array", protocol_version="0.1.0"):
    ...
    if protocol_version != '0.1.0': raise ValueError('Unsupported protocol version')
...
def main():
    ...
    p.add_argument('--preset',choices=PRESETS,default='json-id-array');p.add_argument('--protocol-version',choices=('0.1.0',),default='0.1.0');p.add_argument('--output',type=Path,required=True);p.add_argument('--example',action='store_true');p.add_argument('--check',action='store_true');a=p.parse_args()
